...
2. 选择你的操作系统版本下载
   ...
   - macOS：`WorkBuddy.dmg`
3. 正常安装流程，一路下一步
4. 下载好后进入 WorkBuddy 首页，点击登录

![下载好后进入 WorkBuddy 首页，点击登录](images/workbuddy-login.png)
...
![点击左下角个人信息（头像），在弹出框点击 Claw 设置](images/workbuddy-claw-settings.png)
...
1. 在集成列表中找到「**微信客服号集成**」
2. 点击右侧的「**配置**」按钮，然后下一步完成扫码

![在微信客服号集成右边点击【配置】，然后下一步完成扫码即可](images/wechat-config-scan.png)

3. 扫码成功后绑定完成

### Step 4：开始使用

绑定完成后，微信就会弹出 WorkBuddy 的客服号了，你就可以直接在上面和你的 WorkBuddy 机器人开始聊天了：

![微信弹出 WorkBuddy 客服号，直接开始聊天](images/wechat-chat.png)

例如你可以在手机上让它帮你查看管理桌面的文件，还可以配置邮箱让它把某个文件发送到邮箱里，又或者让它对你的文件做一些操作，例如做一份完整的数据分析然后发送给你：

![WorkBuddy 使用示例](images/workbuddy-demo.jpg)
